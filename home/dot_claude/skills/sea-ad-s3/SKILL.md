...
3. **Load the reference** — read
   [references/buckets-and-routing.md](references/buckets-and-routing.md) for the
   full bucket table, sensitivity rules, and routing examples.
4. **Propose a URI** — construct the path using the prefix schema in
   [references/paths-and-tags.md](references/paths-and-tags.md). State explicitly
   that you have not verified the path exists.
...
- When you encounter a known naming or tagging inconsistency (see
  [references/paths-and-tags.md](references/paths-and-tags.md)), preserve any
  existing observed convention or ask the user to confirm — never silently pick
  one variant.
